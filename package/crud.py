...
def create_user(db: Session, user: UserCreate):
    db_user = User(username=user.username, hashed_password="fakehash")  # Hash password here
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def create_booking(db: Session, booking: BookingCreate):
    ...
    if not user:
        user = create_user(db, UserCreate(username=booking.username, password="fakehash"))
    new_booking = Booking(user_id=user.id, barber_id=booking.barber_id, appointment_time=booking.appointment_time)
    db.add(new_booking)
    db.commit()
